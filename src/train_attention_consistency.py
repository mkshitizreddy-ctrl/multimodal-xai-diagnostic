...
augmentation disabled to keep masks spatially aligned with images —
this script overrides train.augmentation to false regardless of what's in
the config, and warns if the config didn't already say so, rather than
...
def train_one_epoch(
    ...
    for images, _tabular, labels, lung_masks in tqdm(
        loader,
        desc="train",
        leave=False,
    ):
        ...
        consistency = attention_consistency_loss(
            attention_map,
            lung_masks,
        )
...
def main():
    ...
    parser.add_argument(
        "--train-config",
        default="configs/vision_attention_consistency.yaml",
    )

    parser.add_argument(
        "--lung-masks",
        default="data/processed/lung_masks.pkl",
    )
    ...
    print(
        f"attention_consistency_weight = "
    ...
    for epoch in range(
        ...
        print(
            ...
            f"(bce={train_bce:.4f}, "
            f"consistency={train_consistency:.4f}) | "
            f"val_loss={val_metrics['val_loss']:.4f} | "
            f"val_macro_auroc="
            f"{val_metrics['val_macro_auroc']:.4f}"
        )
        ...
        if current_metric > best_metric:
            ...
            if (
                ...
                print(
                    ...
                    f"{train_cfg['train']['early_stopping_patience']} "
                    f"epochs)."
                )
                break
    ...
    print(
        f"Metrics logged to {metrics_csv}"
    )
