...
class ChestXrayExplainer:
    ...
    def explain(
        self, image_tensor: torch.Tensor, class_idx: int, restrict_to_lungs: bool = False
        ...
        Args:
            ...
            restrict_to_lungs: if True, zeroes out heatmap activation
                outside the segmented lung fields before generating the
                overlay. Directly mitigates the shortcut-learning risk
                documented in docs/ethics_statement.md — some Grad-CAM
                outputs were observed activating on shoulders, image
                borders, or burned-in annotations rather than lung tissue.
                Requires torchxrayvision (see src/explain/lung_segmentation.py).
        ...
        if restrict_to_lungs:
            from src.explain.lung_segmentation import get_lung_mask

            lung_mask = get_lung_mask(image_tensor, output_size=grayscale_cam.shape[0])
    ...
    def explain_top_k(
        ...
        for idx in top_indices:
            overlay, heatmap = self.explain(image_tensor, idx, restrict_to_lungs=restrict_to_lungs)
            results.append(
                {
                    "class": classes[idx],
