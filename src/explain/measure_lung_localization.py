...
src/explain/fusion_wrapper.py and generate_fusion_examples.py, which uses the
same per-image-wrapper pattern) - reusing one wrapper across images would
explain every image using some OTHER patient's fixed vitals, which would
...
def main():
    ...
    parser.add_argument(
        "--output-csv",
        default=None,
        help="Optional path to save per-image results.",
    )
